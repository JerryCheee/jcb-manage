...
import { confirmPwd, accountValidator, makeUploadDefaultValue } from '../../../utils/formHelp';
...
const accountItems = (
    <>
        <Item label="密码" name="password" rules={rules.password} hasFeedback><Input.Password /></Item>
        <Item
            name="confirm"
            label="确认密码"
...
export default function Admin({ location, history }) {
    ...
    useEffect( () => {
        const initRoles = async () => {
            ...
            if (res.code){message.error(res.msg); return; }
            setRoles(res.data)
        }
        ...
        >
            ...
            <Item label="角色" name="roleId">
                <Select placeholder="点击选择">
                    {roles.map(v => <Option value={v.id} key={v.id}>{v.name}</Option>)}
                </Select>
            </Item>
            <Item label="账号" name="username" rules={rules.username}><Input /></Item>
