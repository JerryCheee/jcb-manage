...
const validRoleTypes = [1, 3]//门店和城市运营中心

const conditionGet = (isDraft, isVerify) => {
    if (isVerify) return workApi.getDetail
...
export default function ModifyProduct({ location, history }) {
    ...
    const onSubmit = async () => {
        if (!validRoleTypes.includes(curUser.roleType)) return message.error('请使用门店类型的账号')
    ...
    const Intents = useMemo(() => {
        if (isVerify) {
            return (<>
                ...
                <Row>
                    <Col span={3} offset={8}><Button type="primary" onClick={onSubmit}>立即提交</Button></Col>
                </Row>
        ...
        return (
            <Row>
                <Col span={3} offset={8}><Button onClick={onSave}>保存草稿</Button></Col>
                <Col span={3}><Button type="primary" onClick={onSubmit}>立即提交</Button></Col>
            </Row>
        )
    }, [isVerify])
    return (
        <div>
            ...
            <Lead>4. 商品详情</Lead>
            <div style={mb14}>
                <RichTextEditor ref={editorRef} value={detail?.productBase?.detail} />
            </div>
            {Intents}
        </div >
    )
