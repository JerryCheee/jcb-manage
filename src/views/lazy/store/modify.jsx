...
import { message, Form, Input, Button, TimePicker, Checkbox, Select, Row, Col } from "antd";
...
const imgTypeKeys = ["certificatePics", "businessLicensePic", "detailsPic", "head"]
...
const extraFromValues = (detail, values) => {
    ...
    let { businessHours, certificateNo, contacts, address, delTypes = [] } = values
    //storeBaseVo
    //eslint-disable-next-line
    let updated = sameKeys.reduce((pre, k) => (pre[k] = values[k], pre), storeBaseVo)
    delTypes.forEach(c => {
        updated[c] = 1
    })
    updated.delTypes = undefined
    const [openHours, closeHours] = (businessHours || []).map(h => h.format('HH:mm:ss'))
    ...
    //storeBankList
    let { storeBankList: sbl } = values;
    const isEmptyObj = b => Object.values(b).every(v => v !== undefined);
    //eslint-disable-next-line
    detail.storeBankList = sbl.filter(isEmptyObj).map((v, i) => (v.type = i, v))
    return detail
...
const backfill = (data) => {
    ...
    const { openHours, closeHours } = storeBaseVo
    if (openHours && closeHours) {
        values.businessHours = [openHours, closeHours].map(h => moment(h))
    }

    values.delTypes = deliveryTypes.map(([k]) => values[k] ? k : undefined).filter(k => k !== undefined)

    values.storeBankList = data.storeBankList

    return values;
}

export default function ModifyStore({ history, location }) {
    ...
    const onSubmit = async () => {
        ...
        let res = await api.edit(editData.id, values)
        if (res.code) return message.error(res.msg)
        message.success(res.msg, history.goBack)
    }
    useEffect(() => {
        getDetail(editData.id, (data) => {
    ...
    return (
        ...
        >
            ...
            <Item label="登录密码" name="password"><Input placeholder="不填则与会员密码相同" /></Item>
            <Item label="门店等级" name="level">
                <Select disabled={curUser.roleType > 0}>
                    {levels.map(v => <Option key={v.id} value={v.id}>{v.name}</Option>)}
                </Select>
            </Item>
            <Item label="门店名称" name="name" rules={rules.name}><Input /></Item>
            <Item label="联系人" name="contacts" rules={rules.contacts}><Input /></Item>
            <Item label="门店简介" name="introduce" rules={rules.introduce}><Input.TextArea /></Item>
            <Item label="门店地址" name="baseAddress" rules={rules.baseAddress}><Input disabled /></Item>
            <Item label="详细地址" name="address" rules={rules.address}><Input /></Item>
            <Item label="手机号码" name="phone" rules={rules.phone}><Input onChange={onPhoneChange} /></Item>
            <Item label="营业时间" name="businessHours" rules={rules.businessHours}><RangePicker /></Item>
            <Item label="身份证" name="certificateNo" rules={rules.certificateNo}><Input /></Item>
            ...
            <Item label="银行账号" name={["storeBankList", 0, "bankNo"]} ><Input /></Item>
            <Item label="银行户名" name={["storeBankList", 0, "bankName"]} ><Input /></Item>
            <Item label="开户行" name={["storeBankList", 0, "bankAddress"]} ><Input /></Item>
            <Item hidden name={["storeBankList", 0, "id"]} ><Input /></Item>
            ...
            <Item label="银行账号" name={["storeBankList", 1, "bankNo"]} ><Input /></Item>
            <Item label="银行户名" name={["storeBankList", 1, "bankName"]} ><Input /></Item>
            <Item label="开户行" name={["storeBankList", 1, "bankAddress"]} ><Input /></Item>
            <Item hidden name={["storeBankList", 1, "id"]} ><Input /></Item>

            <Item name="delTypes" label="配送类型" wrapperCol={{ span: 20 }}>
                <Checkbox.Group>
