...
const values2data = values => {
  let { rangeTime: [start, end] } = values
...
export default function ModifyCoupons({ location, history }) {
  ...
  return (
    <Form
      ...
      name='coupon'
    >
      <Item label="优惠名称" name="name" rules={rules.name}><Input /></Item>
      ...
      <Item label="优惠方式" name="typeNum" initialValue={0}>
        <Radio.Group>
          {couponTypes.map((v, i) => <Radio key={i} value={i}>{v}</Radio>)}
        </Radio.Group>
      </Item>
      <Item
        noStyle
      ...
      <Item label="使用范围" name="scope" initialValue={0} >
        <Radio.Group>
          {couponScopes.map((v, i) => <Radio key={i} value={i}>{v}</Radio>)}
        </Radio.Group>
      </Item>
      <Item
        noStyle
