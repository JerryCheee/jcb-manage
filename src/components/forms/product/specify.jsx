...
import StoreSku from './store/sku';
...
function Sku({ defo, target }, ref) {
    ...
    const addOneSpesify = (add, index) => () => {
        ...
        validOpts[index] = validOpts[0].filter(v => !ids.includes(v.id))
    ...
    const tableForm = useMemo(() => {
        if (target === 2) return <SkuForm options={selecteds} spec={formValues} defo={defo} ref={ref} />
        if (target === 1) return <StoreSku options={selecteds} spec={formValues} defo={defo} ref={ref} />
    }, [selecteds, formValues])
