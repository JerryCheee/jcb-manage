...
import { genImgUid, makeUploadDefaultValue } from "../../../utils/formHelp";
...
const backfill = (defo) => {
    ...
    productMedias.forEach((v, i) => {
        imgs.push(v.resource);
        v.id = v.id || genImgUid(i); //草稿的时候它没有值
        ids.push(v.id);
        if (v.mediaType === 2) {
            mainImg = v.id;
        }
...
function ProductBase({ defo, target }, ref) {
    ...
    return (
        ...
        >
            <Form {...layout} form={form} name="basicForm">
                {targetItem}
                <Item
                    label="商品图片"
                    name="preview"
                ...
                <Item
                    noStyle
                    shouldUpdate={(pre, cur) => pre.preview !== cur.preview}
                >
                    {({ getFieldValue }) => {
                ...
                <Item label="型号" name="modelName" rules={rules.modelName}>
                    <Input />
                </Item>
                <Item label="名称" name="subname">
                    <Input />
                </Item>
                <Item label="装箱规格" name="packageUnit">
                    <InputNumber style={expandWidth} min={1} />
                </Item>
                <Item label="商品条形码" name="barCode">
                    <Input />
                </Item>
                <Item label="起拍数量" name="clapNumber">
                    <InputNumber style={expandWidth} min={1} />
                </Item>
            </Form>
            <ModalForm
