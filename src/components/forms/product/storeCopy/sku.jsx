...
const StyledImg = styled.img`
     ...
     margin: 0 auto;
`
const ImgShower = ({preview}) => <StyledImg src={preview} />
...
const inputCols = [
    { key: 'advicePrice', name: '市场价', disabled: true },
    { key: 'factoryPrice', name: '供货价', disabled: true },
    { key: 'discount', name: '折扣%', disabled: false },
    { key: 'paymentPrice', name: '售价', disabled: false },
    { key: 'number', name: '库存', disabled: false },
    { key: 'weight', name: '重量', disabled: true },
    { key: 'volume', name: '体积', disabled: true },
    { key: 'skuCode', name: '编号', disabled: true }
];
const shortcutCols = [
...
function SkuForm({ options, spec = defoSpes, defo }, ref) {
    ...
    let contents = items.map((s, i) => (
        <tr key={i}>
            ...
            {inputCols.map((n, ni) => <td key={ni}><Item noStyle name={['skus', i, n.key]}><Input disabled={n.disabled} /></Item></td>)}
            <td>
                {buildSpecItem(s, i)}
                <Item noStyle name={['skus', i, 'preview']} valuePropName="preview"><ImgShower /></Item>
